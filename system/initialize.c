...
uint32 currcputime=0; // LIN, Yi: Initialized to 0
...
local process	startup(void)
{
    ...
    // LIN, Yi: Set main priority to 99 for Benchmark A
    resume(create((void *)main, INITSTK, 99,
					"Main process", 0, NULL));
...
static	void	sysinit()
{
    ...
    // For the idle process (priority 0):
    xstds[0].xstslpret=0;
    xstds[0].xstqexp=0;
    ...
    for (i=1;i<=7;i++)
    {
        xstds[i].xstslpret=(i+1>7)? 7 : i+1;
        xstds[i].xstqexp=(i-1<1)? 1 : i-1;
